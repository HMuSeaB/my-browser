...
- **深度指纹混淆与一致性配对**：基于 Camoufox 内核，并结合真实 Telemetry 数据集提供指纹一致性配对池，杜绝 WebGL 显卡、分辨率和 Canvas 噪音的 Lie 统计学冲突。
- **隐藏 WebDriver 调试痕迹**：通过偏好设置在启动时自动化覆写配置（如 `dom.webdriver.enabled`、`marionette.enabled` 等），有效降低暴露调试接口后的指纹特征泄露。
...
- MyBrowser Pro 内置了本地开发者 API，可在 `Automation` 侧边栏中查看实际监听地址、Bearer token、当前活动会话以及 BiDi 连接地址。
...
- 日常简单操作可以直接在 Automation 面板里选环境、贴链接、一键打开；这条 UI 链路与下方 Python 示例走的是同一套 BiDi 导航流程。Python 示例则保留给批量、多步、联动脚本等高级场景。
...
MyBrowser Pro 提供了开箱即用的行为拟真算法以及无人值守的自动化测试套件：

*   **拟真控制示例**：在 [examples/bidi_stealth.py](examples/bidi_stealth.py) 中，展示了通过 BiDi 执行二阶贝塞尔曲线鼠标平滑滑动、模拟物理持载时间点击以及变频延迟打字的交互方式。
*   **防检测自动检测套件**：在 [scripts/verify_stealth.py](scripts/verify_stealth.py) 中，提供了一键式自动化审计脚本，可无人值守完成 `navigator.webdriver` 隐藏检测、WebGL/Canvas 扰动校验，并能在对照模式下进行在线 reCAPTCHA 自动跑分。运行命令如下：
    ```bash
    # 1. 离线指纹与调试端口暴露审计
    python scripts/verify_stealth.py --token YOUR_TOKEN --profile-id YOUR_PROFILE_ID

    # 2. 包含在线 CreepJS Lies 检测及 reCAPTCHA 自动跑分的交叉对照评测
    python scripts/verify_stealth.py --token YOUR_TOKEN --profile-id YOUR_PROFILE_ID --online
    ```
